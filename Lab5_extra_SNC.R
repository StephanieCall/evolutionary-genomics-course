...
# Import the necessary packages and data for the lab
library(tidyverse)
library(lubridate)
...
time_series_deaths_long <- time_series_deaths_long %>% 
  ...
  select(Key, Deaths)
# Check that both tables look proper
...
# show the code. Alternatively, code = FALSE will exectute the code but not show the code. 
# You can also suppress error messages and warnings so the report won't include them 
# (although you should still be aware that they exist).
...
ppi <- 300
png('images/time_series_example_plot.png', width = 6*ppi, height = 6*ppi)
time_series_long_joined %>% 
  group_by(Country_Region, Date) %>% 
...
# Subset hte time series data to include the US deaths
...
# shadow_*() functions define how data from other points in time should appear at a given 
# point in time during the animation.
# enter_*() and exit_*() functions define how new data should appear 
# and old data disappear in the animation, respectively.
