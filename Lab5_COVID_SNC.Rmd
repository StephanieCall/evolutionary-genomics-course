...
---
# Lab 5 - More Data Wrangling and Graphing COVID-19 Reporting Data Over Time
...
```{r, pivoting, fig.width = 6, fig.height = 10}
...
# Now, plot the data in a scatterplot. Adjust the figure size in Rmd using fig.width and 
...
```{r, downloadTimeSeries, eval = FALSE}
# To save the time series data to the computer as a sort of snapshot - 
# Confirmed cases - 
...
```{r timeSeriesDeaths}
# Read downloaded table to R and rename - 
...
Join the two time series tables using a common column with a unique key (name) - 

```{r joinTimeSeries}
# Currently, no column has a unique key,but we can make one by merging data
# in the Province_State, Country_Region, and Date columns using unite() - 
...
## Exercise 1 
See the Lab4_ggplot_tutorial file to find the rest of the ggplot tutorial


## Exercise 2 
Join tables like above for the 6/13/2020 and 9/13/2020 data, then make a bar graph of the data

```{r Q2, fig.width=9, fig.height=8}
...
## Exercise 3
Add a title and more informative x and y axes labels to the plot in Exercise 2

```{r Q3, fig.width=9, fig.height=8}
advanced_confirmed_states_plot <- basic_confirmed_states_plot +
  labs(x = "US State/Territory", y = "Number of Confirmed Cases", title = "Total number of Confirmed COVID-19 Cases by State in June and September") +
...
## Exercise 4
Using the time series data, plot the total number of worldwide confirmed deaths per day over time
...
## Exercise 5
Use mutate() to add a new column of deaths/confirmed cases to the time series data
...
## Exercise 6
Plot the US deaths/confirmed cases over time
...
## Exercise 7
Make a single graph of deaths over time for the 10 countries with the highest total deaths

```{r Q7, fig.width=8, fig.height=7}
...
## Exercise 8
Use facet_wrap() to graph the data above in their own little graphs, adjusting the settings to make the graph look better

```{r Q8, fig.width=9, fig.height=8}
...
## Exercise 9
Use facet_wrap() to graph the time series data for the confirmed cases in all US states and territories over time. Format the graphs to look nice again.

```{r Q9Download, eval=FALSE}
...
```
```{r Q9, fig.width=11, fig.height=20}
...
# Now plot the data
plot_time_series_confirmed_States <- ggplot(time_series_confirmed_US, aes(x = Date, y = Total_Confirmed, group = Province_State)) + # Must gropu by state to graph properly
  geom_point() +
  geom_line() +
...
## Exercise 10
Apply a theme other than those used in the Data Carpentry tutorial to the graph in Exercise 10

```{r Q10, fig.width=9, fig.height=8}
